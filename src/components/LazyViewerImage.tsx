...
const LazyViewerImage = ({ src, imageNum, style }: Props) => {
  ...
  useEffect(() => {
    ...
    if (!intersectionObserverRef.current) intersectionObserverRef.current = new IntersectionObserver(intersectionCallback, { threshold: 0.5 })
    if (intersectionObserverRef.current && imgRef.current) intersectionObserverRef.current.observe(imgRef.current) 
  }, [ imgRef ])

  return (
    !isIntersect || isLoading ? <Flex ref={imgRef} alignItems="center" justifyContent="center" minHeight="100vh"><Loading /></Flex> : <img
    style={style}
